...
For a lease you hold, add `--devices` to connect to its exporter and discover
its drivers and runnable `j` commands:

```console
$ jmp describe lease 01a05822-e378-71cc-a98c-a216ad4a9432 --client hello --devices
$ jmp describe lease 01a05822-e378-71cc-a98c-a216ad4a9432 --client hello --devices -o json
```

The human-readable description adds **Devices** and **Commands** tables. With
`-o json` or `-o yaml`, the result is `{lease, devices}`: `lease` contains the
usual lease metadata, while `devices` contains a `drivers` list and the recursive
...
and uses the selected client's driver-access settings. Without `--devices`,
