...
func LoadConfiguration(
...
) (authenticator.Token, string, Router, []grpc.ServerOption, *Provisioning, *LeasePolicy, *HiddenLabels, *DeprecatedLabels, *Telemetry, error) {
	var configmap corev1.ConfigMap
	if err := client.Get(ctx, key, &configmap); err != nil {
	...
	if !ok {
		return nil, "", nil, nil, nil, nil, nil, nil, nil, fmt.Errorf("LoadConfiguration: missing router section")
	}
	...
	if ok {
		...
		}, &Provisioning{Enabled: false}, &LeasePolicy{MaxTags: 10}, nil, nil, nil, nil
	}

	rawConfig, ok := configmap.Data["config"]
	if !ok {
		return nil, "", nil, nil, nil, nil, nil, nil, nil, fmt.Errorf("LoadConfiguration: missing config section")
	}
	...
	if config.Telemetry != nil && config.Telemetry.Endpoint != "" {
		telemetry = config.Telemetry
	}
	return authenticator, prefix, router, serverOptions, &config.Provisioning, &config.LeasePolicy, &config.HiddenLabels, &config.DeprecatedLabels, telemetry, nil
}
