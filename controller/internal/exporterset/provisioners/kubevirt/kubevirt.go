...
func (p *Provisioner) EnrichExporterExport(
	...
	if !hasKubevirt {
		...
		storageCfg := maps.Clone(baseCfg)
		storageCfg["disk_size"] = params.DiskSize
		storageCfg["access_modes"] = []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce}
		if params.Storage != nil && len(params.Storage.AccessModes) > 0 {
			storageCfg["access_modes"] = params.Storage.AccessModes
		}

		result = append(result,
			virtualtargetv1alpha1.DriverConfig{Name: "power", Type: kubevirtPowerType, Config: mustJSON(baseCfg)},
			virtualtargetv1alpha1.DriverConfig{Name: "console", Type: kubevirtConsoleType, Config: mustJSON(baseCfg)},
			virtualtargetv1alpha1.DriverConfig{Name: "storage", Type: kubevirtStorageType, Config: mustJSON(storageCfg)},
		)

		if params.SSHAccess != "" {
			sshHost, sshPort, err := p.resolveServiceAddress(ctx, vtc, params, exporter.Name+"-ssh", 22000)
			if err != nil {
				return nil, fmt.Errorf("resolve SSH service address: %w", err)
			}
			if sshHost != "" {
		...
		for name, port := range params.ForwardPorts {
			...
			host, resolvedPort, err := p.resolveServiceAddress(ctx, vtc, params, exporter.Name+"-"+name, int32(port))
			if err != nil {
				return nil, fmt.Errorf("resolve forwarded port %q service address: %w", name, err)
			}
			if host != "" {
				result = append(result, virtualtargetv1alpha1.DriverConfig{
					Name:   name,
...
// an address reachable from the management cluster.
func (p *Provisioner) resolveServiceAddress(
	...
	if svc.Spec.Type == corev1.ServiceTypeLoadBalancer {
		for _, ing := range svc.Status.LoadBalancer.Ingress {
			if ing.IP != "" {
				return ing.IP, port, nil
			}
			if ing.Hostname != "" {
				return ing.Hostname, port, nil
			}
		}
		return "", 0, fmt.Errorf("LoadBalancer service %s/%s has no ingress address", params.Namespace, svcName)
	}

	if svc.Spec.Type == corev1.ServiceTypeNodePort {
		host, err := resolveNodeAddress(ctx, remoteClient)
		if err != nil {
			return "", 0, err
		}
	...
	return "", 0, fmt.Errorf("service %s/%s has no reachable address", params.Namespace, svcName)
}

func resolveNodeAddress(ctx context.Context, remoteClient client.Client) (string, error) {
	nodes := &corev1.NodeList{}
	if err := remoteClient.List(ctx, nodes); err != nil {
		return "", fmt.Errorf("list nodes for NodePort service: %w", err)
	}
	...
	return "", fmt.Errorf("no node address available for NodePort service")
}
...
func (p *Provisioner) Cleanup(
	...
	if vtc.Spec.ReclaimPolicy == virtualtargetv1alpha1.ReclaimPolicyRetain {
		return nil
	}
