...
type TelemetryConfig struct {
	...
	// Note: when replicas is set to 0, the controller also removes the telemetry
	// endpoint from the controller ConfigMap, so exporters will stop forwarding
	// metric streams immediately. Any exporter that was already streaming metrics
	// will lose its MetricsStream target until telemetry is resumed (replicas > 0)
	// and the ConfigMap is reconciled again.
	// +kubebuilder:default=1
	// +kubebuilder:validation:Minimum=0
	Replicas *int32 `json:"replicas,omitempty"`
...
type RoutersConfig struct {
	...
	// Note: when replicas is 0, router entries are also removed from the controller ConfigMap,
	// so the running controller will see no router endpoints. Any Dial/Listen attempt will fail
	// until routers are resumed (replicas > 0) and the ConfigMap is reconciled again.
