...
// Rust workspace. The cdylib is built ONCE, by the runtime module (`:jumpstarter-client`, which owns
// the bindings); every other module's tests just depend on that single task and put it on the JNA
// library path. The cdylib path + rust dir are exposed as extra properties so the runtime module can
// wire its own cargoBuildCore + UniFFI/codegen tasks off the same locations.
...
// The cdylib-producing task lives in the runtime module; reference it by path (lazy, so it resolves
// even though this convention also applies to that module before the task is registered).
val cargoBuildCorePath = ":jumpstarter-client:cargoBuildCore"
