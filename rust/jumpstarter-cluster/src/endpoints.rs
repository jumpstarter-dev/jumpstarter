...
pub async fn get_ip_generic(cluster_type: Option<&str>, minikube: &str, cluster_name: &str) -> Result<String> {
    if cluster_type == Some("minikube") {
        if !minikube_installed(minikube) {
            return Err(ClusterError::tool_not_installed("minikube"));
        }
        get_minikube_ip(cluster_name, minikube)
            .await
            .map_err(|e| ClusterError::Endpoint(format!("Could not determine Minikube IP address.\n{e}")))
    } else {
        let ip = get_ip_address();
        if ip == "0.0.0.0" {
            return Err(ClusterError::Endpoint(
                "Could not determine IP address, use --ip <IP> to specify an IP address".to_string(),
            ));
        }
        Ok(ip)
    }
...
pub async fn configure_endpoints(
    ...
    let grpc_endpoint = grpc_endpoint.unwrap_or_else(|| {
        let port = if cluster_type == Some("k3s") { GRPC_NODEPORT } else { KIND_GRPC_HOST_PORT };
        format!("grpc.{basedomain}:{port}")
    });
    let router_endpoint = router_endpoint.unwrap_or_else(|| {
        let port = if cluster_type == Some("k3s") { ROUTER_NODEPORT } else { KIND_ROUTER_HOST_PORT };
...
mod tests {
    ...
    async fn k3s_uses_nodeports() {
        let (_, _, grpc, router) =
            configure_endpoints(Some("k3s"), "minikube", "c", Some("10.0.0.1".to_string()), None, None, None)
                .await
                .unwrap();
