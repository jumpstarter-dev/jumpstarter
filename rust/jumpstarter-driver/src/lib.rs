...
/// Re-exported so the `#[driver]`-generated registrations can reach `inventory::submit!`. (The
/// client-side `#[client_cli]`/`client_main!` entrypoint now lives in `jumpstarter-client`.)
#[doc(hidden)]
pub use inventory;
...
// The client-side entrypoint (`Client`/`#[client_cli]`/`client_main!`) lives in `jumpstarter-client`
// — a driver crate's host bin imports its host glue from here, and a client crate imports its CLI
// glue from `jumpstarter-client`, keeping the two sides disjoint.
