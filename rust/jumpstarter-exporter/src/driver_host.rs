//! The slim Python driver-host subprocess (spec 09 §3.2, option a; native-migration
//! design `rust/docs/03-native-exporter-migration.md`).
//!
//! [`SlimHost`] spawns `slim_driver_host.py`, which instantiates the whole driver
//! tree and serves the *driver-level* RPCs (`DriverCall`/`StreamingDriverCall`/
//! `Stream`/`GetReport`/`LogStream`) on a **single** private socket. The Rust core
//! owns the client/hook-facing protocol and proxies driver calls into it by UUID.
//! The host prints its socket path on stdout and is torn down when dropped.
...
/// The embedded Python host script (written to a temp file at spawn time so the Rust
...
/// How long to wait for the driver host to report its socket path.
...
impl SlimHost {
    ...
    pub async fn spawn(config_path: &Path) -> Result<Self, Error> {
        let (child, mut lines) = spawn_host(SLIM_HELPER, config_path)?;
...
/// Write the embedded helper to a temp file and spawn the configured Python
/// interpreter on it, returning the child and a line reader over its stdout.
fn spawn_host(helper: &str, config_path: &Path) -> Result<(Child, StdoutLines), Error> {
    let script = std::env::temp_dir().join(format!("jmp-slim-host-{}.py", std::process::id()));
    std::fs::write(&script, helper)
        .map_err(|e| Error::Config(format!("writing slim host helper: {e}")))?;
    ...
    let mut child = Command::new(&python)
        ...
        .kill_on_drop(true)
        .spawn()
        .map_err(|e| Error::Config(format!("spawning slim host ({python}): {e}")))?;

    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| Error::Config("slim host has no stdout".into()))?;
...
async fn next_socket_line(lines: &mut StdoutLines) -> Result<String, Error> {
    lines
        .next_line()
        .await
        .map_err(|e| Error::Config(format!("reading slim-host socket: {e}")))?
        .ok_or_else(|| Error::Config("slim host exited before reporting its socket".into()))
}

/// Forward any further host stdout to the log instead of letting the pipe fill
/// (stderr is inherited, so Python logs are already visible).
