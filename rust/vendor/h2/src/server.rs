...
impl proto::Peer for Peer {
    ...
    */
    ...
    fn convert_poll_message(
        ...
        // header
        //
        // JUMPSTARTER PATCH: tolerate an unparseable `:authority` by treating it as absent
        // instead of resetting the stream as malformed. Legacy gRPC C-core clients (e.g.
        // grpc-python <= 1.x dialing `unix:///path`) set `:authority` to the percent-encoded
        // socket path ("var%2Ffolders%2F...%2Fsocket"); `http::uri::Authority` rejects `%`,
        // which killed every RPC from an old Jumpstarter client tunneled to the Rust exporter
        // (RST_STREAM PROTOCOL_ERROR). gRPC servers do not route on the authority, so dropping
        // it is safe. This is the only change in this vendored copy of h2.
