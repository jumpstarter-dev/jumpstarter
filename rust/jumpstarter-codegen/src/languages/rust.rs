...
impl LanguageGenerator for RustGenerator {
    ...
    fn generate_driver(&self, iface: &InterfaceRef) -> BTreeMap<String, String> {
        ...
        // author renames and fills in, then serves via `jumpstarter_driver::serve_driver`.
...
/// complete, the generic `jumpstarter_driver::serve_driver` serves it.
...
// service served by `jumpstarter_driver::serve_driver`; nothing here is generated. Kept
...
fn render_client(iface: &InterfaceRef) -> String {
    ...
    // interface(s) explicitly via the `jumpstarter_driver::Host` / `::Client`
