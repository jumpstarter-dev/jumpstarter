...
fn render_client(iface: &InterfaceRef) -> String {
    ...
    // A generated `<short>_host!(driver)` macro — the crate's whole `src/main.rs` in one line, with
    // NO visible wrapper: the client class, descriptor, and `tonic` server type are baked in here, so
    // the author passes only their driver instance. `#[macro_export]` hoists it to the crate root
    // (`<crate>::<short>_host!`); it expands to a `fn main` calling `run_host`.
    let server_mod = format!("{}_server", pascal_to_snake(service));
    let server_type = format!("{service}Server");
    let short = pascal_to_snake(&strip_interface_suffix(service));
    let client_class = format!(
        "jumpstarter_driver_{}.client.{}Client",
        short,
        strip_interface_suffix(service)
    );
    let _ = writeln!(
        s,
        "/// Serve this crate's `{service}` driver as a STANDALONE host — the whole `src/main.rs`:\n\
         /// `<crate>::{short}_host!(MyDriver::default());`. The client class, descriptor, and server\n\
         /// type are baked in, so only the driver instance is named. Generated by jumpstarter-codegen.\n\
         #[macro_export]\n\
         macro_rules! {short}_host {{\n    \
             ($driver:expr $(,)?) => {{\n        \
                 ...
                 > {{\n            \
                     ::jumpstarter_driver_runtime::run_host(\n                \
                         ...
                         $crate::proto::{server_mod}::{server_type}::new($driver),\n            \
                     )\n        \
                 }}\n    \
             }};\n\
         }}\n"
    ...
    // A generated `<short>_client!(MyCli)` macro — the whole `src/client.rs` of a driver crate's
    // CLIENT CLI binary (the one `j` spawns), symmetric with `{short}_host!`. It expands to a `fn
    // main` that hands the typed client CLI's `run` to `run_client_cli` (connect, resolve the driver,
    // dispatch) — no per-crate client boilerplate.
    let _ = writeln!(
        s,
        "/// Run this crate's `{service}` CLIENT CLI as a STANDALONE binary — the whole\n\
         /// `src/client.rs`: `<crate>::{short}_client!(MyCli);` where `MyCli::run(args, session,\n\
         /// uuid)` dispatches the subcommands. `j` spawns this binary for the advertised client.\n\
         /// Generated by jumpstarter-codegen.\n\
         #[macro_export]\n\
         macro_rules! {short}_client {{\n    \
             ($cli:ty) => {{\n        \
                 fn main() -> ::std::process::ExitCode {{\n            \
                     ::jumpstarter_core::run_client_cli(|args, session, uuid| {{\n                \
