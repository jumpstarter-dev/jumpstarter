...
//!   `@abstractmethod`s + a `client()` classmethod). The author subclasses it alongside
//!   `jumpstarter.driver.Driver`, fills each method in with `@export`, and the generic native host
//!   (proto_marshal) serves it — the `.proto` is the source of truth.
...
impl LanguageGenerator for PythonGenerator {
    ...
    fn generate_driver(&self, iface: &InterfaceRef) -> BTreeMap<String, String> {
        ...
        out.insert(driver_file_name(iface), render_driver(iface));
        out
    }
...
fn render_driver(iface: &InterfaceRef) -> String {
    ...
    let _ = write!(
        ...
        "# @generated by jumpstarter-codegen (PythonGenerator). DO NOT EDIT.\n\
         ...
         # truth). Subclass it ALONGSIDE jumpstarter.driver.Driver and implement each method with\n\
         # @export; the generic native host (jumpstarter.driver.proto_marshal) serves it. Example:\n\
         #\n\
         #   from jumpstarter.driver import Driver, export\n\
         #   class My{svc_stripped}({service}, Driver):\n\
         #       @export\n\
         #       async def on(self) -> None: ...\n\n\
         from abc import ABCMeta, abstractmethod\n",
        svc_stripped = strip_interface_suffix(service),
    );
    // Imports: AsyncIterator only if any server-streaming method; the referenced models.
    ...
    let _ = write!(
        ...
        "\n\nclass {service}(metaclass=ABCMeta):\n    \
             \"\"\"Proto-first `{service}` driver contract (generated). Implement with @export.\"\"\"\n\n    \
             @classmethod\n    \
             def client(cls) -> str:\n        \
                 return \"{stem}_client.{client}\"\n"
    );
    for m in &iface.methods {
...
mod tests {
    ...
    fn driver_base_is_proto_first_abc() {
        let files = PythonGenerator.generate_driver(&power_iface());
        ...
        assert!(src.contains("class PowerInterface(metaclass=ABCMeta):"), "{src}");
    ...
    fn client_uses_native_seam_and_native_types() {
        let files = PythonGenerator.generate_client(&power_iface());
