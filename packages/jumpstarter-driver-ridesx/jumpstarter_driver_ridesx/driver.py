...
class RideSXDriver(Driver):
    ...
    def _decompress_file(self, compressed_file: Path) -> Path:
        if compressed_file.name.endswith(".xz"):
            decompressed_name = compressed_file.name[:-3]  # Remove .xz
        elif compressed_file.name.endswith(".gz"):
            decompressed_name = compressed_file.name[:-3]  # Remove .gz
        elif compressed_file.name.endswith(".gzip"):
            decompressed_name = compressed_file.name[:-5]  # Remove .gzip
        else:
        ...
        try:
            ...
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True, check=True, timeout=600)
    ...
    def detect_fastboot_device(self, max_attempts: int = 5, delay: float = 2.0):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                result = subprocess.run(['fastboot', 'devices', '-l'],
    ...
    def flash_with_fastboot(self, device_id: str, partitions: Dict[str, str]):
        ...
        for partition_name, filename in partitions.items():
            ...
            try:
                result = subprocess.run(cmd, capture_output=True, text=True, check=True, timeout=500)
