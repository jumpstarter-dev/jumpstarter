...
The example **typed device wrapper**: from the committed [`exporter.yaml`](exporter.yaml), the
`jumpstarter_codegen` build hook generates a device class whose structure mirrors the configured
driver tree — `device.dut.power`, `device.dut.backup_power` — with every node bound to a typed
client. This is what a test project consuming a Jumpstarter exporter looks like: **no
...
- `device.py` — `ExampleRigDevice(root)` mirroring the config tree
...
|---|---|---|
| `dut.power` | explicit `interface:` key | `CyclingPowerClient` (custom, from `registry.yaml`) |
| `dut.backup_power` | committed `interfaces/registry/` | generated `PowerClient` (no custom client advertised) |
