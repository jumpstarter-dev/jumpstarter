...
def test_config_nucleo_h753zi_openbsw_zephyr_yaml_validates_driver_instances():
    data = yaml.safe_load((EXAMPLES_DIR / "config_nucleo_h753zi_openbsw_zephyr.yaml").read_text())
    for _name, driver_data in data["export"].items():
        ExporterConfigV1Alpha1DriverInstance.model_validate(driver_data)


def test_config_nxp_s32k388_opensomeip_zephyr_yaml_validates_driver_instances():
