...
def build_registry(drivers: dict[str, type], version: str | None = None) -> dict:
    ...
    Per driver: the interface FQN + proto path derived exactly the way the exporter host
    advertises it at runtime (``resolve_interface_class_of(cls) or cls`` →
    ``build_file_descriptor``), and the advertised custom client. Labels pointing into a
    ...
    registry_drivers: dict[str, dict] = {}
    ...
    for type_path, cls in sorted(drivers.items()):
        ...
        interfaces[fqn] = _proto_path_for(fd).replace(os.sep, "/")

        entry: dict = {"interface": fqn}
        try:
            label = cls.client()
        except Exception:  # noqa: BLE001 — e.g. Proxy.client() raises by design
            label = None
        if label and "._generated." not in label:
            entry["clients"] = {_client_language(label): label}
        registry_drivers[type_path] = entry

    return {"version": 1, "drivers": registry_drivers, "interfaces": interfaces}


def render_registry_yaml(registry: dict) -> str:
    """Render the registry deterministically as YAML (no pyyaml dependency).

    All keys and string values are double-quoted — driver types like ``rust:power`` contain
    ``:`` and would otherwise need case-by-case escaping.
    """
    ...
    for type_path, entry in sorted(registry["drivers"].items()):
        lines.append(f"  {q(type_path)}:")
        lines.append(f"    interface: {q(entry['interface'])}")
        clients = entry.get("clients") or {}
        if clients:
            lines.append("    clients:")
            for lang, label in sorted(clients.items()):
                lines.append(f"      {lang}: {q(label)}")
    lines.append("interfaces:")
    for fqn, proto in sorted(registry["interfaces"].items()):
        lines.append(f"  {q(fqn)}:")
        lines.append(f"    proto: {q(proto)}")
    return "\n".join(lines) + "\n"
...
def _cmd_generate_all(args: argparse.Namespace) -> int:
    ...
    if args.registry_out:
        ...
        print(f"Generated {args.registry_out} ({len(registry['drivers'])} drivers)")
    return 0
