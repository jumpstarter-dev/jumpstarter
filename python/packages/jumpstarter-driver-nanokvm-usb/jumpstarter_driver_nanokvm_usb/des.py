"""RFB VncAuth DES (ECB, 8-byte blocks) with bit-reversed keys.

RFB 3.8 VncAuth (RFC 6143 / RealVNC) encrypts a 16-byte challenge with DES
in ECB mode. The password is truncated or padded to 8 bytes and each byte is
bit-reversed before it is used as the DES key.

The permutation tables and round function follow FIPS 46-3 (and the
public-domain d3des implementation). This is not PyDES.
"""
...
def des_ecb_encrypt(block: bytes, key: bytes) -> bytes:
    """Encrypt one 8-byte block with DES-ECB (FIPS 46-3)."""
...
def vnc_auth_response(challenge: bytes, password: str) -> bytes:
    ...
    return des_ecb_encrypt(challenge[:8], des_key) + des_ecb_encrypt(challenge[8:], des_key)
