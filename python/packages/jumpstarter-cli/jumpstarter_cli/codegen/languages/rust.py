...
def _gen_device_wrapper(ctx: CodegenContext) -> str:
    ...
    for iface in ec.interfaces:
        ...
        if is_external:
            ...
            lines.append(f"use crate::clients::{mod_name}_client::{client_name};")
    lines.append("")
...
class RustLanguageGenerator(LanguageGenerator):
    ...
    def generate_device_wrapper(self, ctx: CodegenContext) -> dict[str, str]:
        ...
        # Generate lib.rs that declares the module tree
        lib_lines = [
            "// Auto-generated by `jmp codegen --language rust`. Do not edit.",
            "",
        ]
        # Only include clients module if we have generated (non-external) clients
        generated_clients = []
        for iface in ec.interfaces:
            _, _, is_external = _resolve_rust_import(iface)
            if not is_external:
                mod_name_client = _pascal_to_snake(iface.service_name).replace("_interface", "")
                generated_clients.append(f"    pub mod {mod_name_client}_client;")
        if generated_clients:
            lib_lines.append("pub mod clients {")
            lib_lines.extend(generated_clients)
            lib_lines.append("}")
            lib_lines.append("")
        lib_lines.append("pub mod devices;")
        lib_lines.append("")
