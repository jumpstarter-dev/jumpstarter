...
def _build_console_ws_url(
    ...
    if parsed.scheme != "https":
        raise ValueError("kubeconfig cluster server must use HTTPS for console connections")
    ws_url = (
        f"wss://{parsed.netloc}/apis/subresources.kubevirt.io/v1/"
            ...
            try:
                ...
                if e.response.status_code == 400:
                    self.logger.info(
                        ...
                        self.console_retry_interval,
                        attempt + 1,
                        self.console_max_retries,
                    )
...
class KubevirtStorage(_KubevirtBase, Driver):
    ...
    delete_poll_interval: float = 0.5
    ...
    async def flash(
        ...
        if deleted:
            async with asyncio.timeout(self.delete_timeout):
                while True:
                    try:
                        await client.get(DataVolume, name=dv_name, namespace=self.namespace)
                    except ApiError as e:
                        if e.status.code == 404:
                            break
                        raise
                    await asyncio.sleep(self.delete_poll_interval)
