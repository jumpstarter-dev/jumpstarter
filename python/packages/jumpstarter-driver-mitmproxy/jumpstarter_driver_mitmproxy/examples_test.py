...
def test_conftest_py_compiles():
    source = (EXAMPLES_DIR / "conftest.py").read_text()
    compile(source, "conftest.py", "exec")


def test_test_device_py_compiles():
    source = (EXAMPLES_DIR / "test_device.py").read_text()
    compile(source, "test_device.py", "exec")
