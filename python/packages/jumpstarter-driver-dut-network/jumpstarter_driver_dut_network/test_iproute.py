...
class TestIptablesForward:
    def test_is_iptables_forward_drop_true(self):
        fake = subprocess.CompletedProcess(args=[], returncode=0, stdout="-P FORWARD DROP\n")
        with patch("shutil.which", return_value="/usr/sbin/iptables"), \
             patch.object(iproute, "_run_priv", return_value=fake):
            assert iproute.is_iptables_forward_drop() is True

    def test_is_iptables_forward_drop_accept(self):
        fake = subprocess.CompletedProcess(args=[], returncode=0, stdout="-P FORWARD ACCEPT\n")
        with patch("shutil.which", return_value="/usr/sbin/iptables"), \
             patch.object(iproute, "_run_priv", return_value=fake):
            assert iproute.is_iptables_forward_drop() is False

    def test_skips_when_iptables_not_installed(self):
        with patch("shutil.which", return_value=None):
            assert iproute.is_iptables_forward_drop() is False

    def test_skips_on_iptables_error(self):
        fake = subprocess.CompletedProcess(args=[], returncode=4, stdout="")
        with patch("shutil.which", return_value="/usr/sbin/iptables"), \
             patch.object(iproute, "_run_priv", return_value=fake):
            assert iproute.is_iptables_forward_drop() is False

    def test_ensure_inserts_rules_when_drop(self):
        with patch.object(iproute, "is_iptables_forward_drop", return_value=True), \
             patch.object(iproute, "_run_priv") as mock:
            rules = iproute.ensure_iptables_forward("br-jmp0", "eth0")
            assert len(rules) == 4
            assert ("-i", "br-jmp0") in rules
            assert ("-o", "br-jmp0") in rules
            assert ("-i", "eth0") in rules
            assert ("-o", "eth0") in rules
            assert mock.call_count == 4

    def test_ensure_returns_empty_when_accept(self):
        with patch.object(iproute, "is_iptables_forward_drop", return_value=False):
            rules = iproute.ensure_iptables_forward("br-jmp0", "eth0")
            assert rules == []

    def test_remove_calls_iptables_delete(self):
        rules = [("-i", "br-jmp0"), ("-o", "br-jmp0")]
        with patch.object(iproute, "_run_priv") as mock:
            iproute.remove_iptables_forward(rules)
            assert mock.call_count == 2
            mock.assert_any_call(
                ["iptables", "-D", "FORWARD", "-i", "br-jmp0", "-j", "ACCEPT"], check=False
            )
            mock.assert_any_call(
                ["iptables", "-D", "FORWARD", "-o", "br-jmp0", "-j", "ACCEPT"], check=False
            )

    def test_remove_noop_with_empty_rules(self):
        with patch.object(iproute, "_run_priv") as mock:
            iproute.remove_iptables_forward([])
            mock.assert_not_called()
